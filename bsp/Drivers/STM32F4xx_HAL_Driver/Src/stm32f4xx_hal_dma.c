            ...
            if (__HAL_DMA_GET_IT_SOURCE (hdma, DMA_IT_HT) != RESET)
                {
                    ...
                    if (((hdma->Instance->CR) & (uint32_t) (DMA_SxCR_DBM))
                        ...
                        {
                            /* Disable the half transfer interrupt if the DMA
            ...
            if (__HAL_DMA_GET_IT_SOURCE (hdma, DMA_IT_TC) != RESET)
                {
                            ...
                            if ((hdma->Instance->CR & DMA_SxCR_CT) == RESET)
                                {
                                    if (hdma->XferM1CpltCallback != NULL)
                                        {
                                            /* Transfer complete Callback for
                                             * memory1 */
                    ...
                    /* Disable the transfer complete interrupt if the DMA mode
                     * is not CIRCULAR */
                    else
                        {
                            if ((hdma->Instance->CR & DMA_SxCR_CIRC) == RESET)
 ...
 */
static void
DMA_SetConfig (DMA_HandleTypeDef *hdma, uint32_t SrcAddress,
               uint32_t DstAddress, uint32_t DataLength)
{
    /* Clear DBM bit */
