...
#include "stm32f4xx_hal_def.h"
    ...
    typedef struct
    {
        uint32_t BaudRate; /*!< This member configures the UART communication
                              ...
                              ((uint32_t) IntegerDivider)) * 8 * (OVR8+1)) + 0.5
        ...
        uint32_t HwFlowCtl; /*!< Specifies whether the hardware flow control
                               mode is enabled or disabled. This parameter can
                               be a value of @ref UART_Hardware_Flow_Control */

        uint32_t OverSampling; /*!< Specifies whether the Over sampling 8 is
                                  enabled or disabled, to achieve higher speed
                                  (up to fPCLK/8). This parameter can be a value
                                  of @ref UART_Over_Sampling */
    } UART_InitTypeDef;
     ...
     * management and also information related to Tx operations. gState value
     * coding follow below described bitmap : b7-b6  Error information 00 : No
     * Error 01 : (Not Used) 10 : Timeout 11 : Error b5     Peripheral
     * initialization status 0  : Reset (Peripheral not initialized) 1  : Init
     * done (Peripheral initialized. HAL UART Init function already called)
     *          b4-b3  (not used)
     *             xx : Should be set to 00
     *          b2     Intrinsic process state
     *             0  : Ready
     *             1  : Busy (Peripheral busy with some configuration or
     * internal operations) b1     (not used) x  : Should be set to 0 b0     Tx
     * state 0  : Ready (no Tx operation ongoing) 1  : Busy (Tx operation
     * ongoing)
    ...
    typedef enum
    {
        ...
        = 0x23U, /*!< Data Transmission and Reception process is ongoing
                     ...
                     Value is result of combination (Or) between gState and
                    RxState values */
     ...
     * @note  HAL UART Reception type value aims to identify which type of
     * Reception is ongoing. This parameter can be a value of @ref
     * UART_Reception_Type_Values : HAL_UART_RECEPTION_STANDARD         = 0x00U,
     ...
     * has occurred leading to call of the RxEvent callback. This parameter can
     * be a value of @ref UART_RxEvent_Type_Values : HAL_UART_RXEVENT_TC =
     * 0x00U, HAL_UART_RXEVENT_HT                 = 0x01U, HAL_UART_RXEVENT_IDLE
     * = 0x02U,
     */
    ...
    typedef struct __UART_HandleTypeDef
    {
        ...
        __IO HAL_UART_StateTypeDef
            gState; /*!< UART state information related to global Handle
    ...
    typedef void (*pUART_RxEventCallbackTypeDef) (
        struct __UART_HandleTypeDef *huart,
        uint16_t
            Pos); /*!< pointer to a UART Rx Event specific callback function */
