...
/* Exported types ------------------------------------------------------------*/
    ...
    typedef struct
    {
        ...
        uint32_t
            Line_0_31; /*!< Specifies the EXTI lines to be enabled or disabled
                          for Lines in range 0 to 31 This parameter can be any
                          combination of @ref EXTI_LL_EC_LINE */

        FunctionalState LineCommand; /*!< Specifies the new state of the
                                        selected EXTI lines. This parameter can
                                        be set either to ENABLE or DISABLE */

        uint8_t
            Mode; /*!< Specifies the mode for the EXTI lines.
                       This parameter can be a value of @ref EXTI_LL_EC_MODE. */

        uint8_t Trigger; /*!< Specifies the trigger signal active edge for the
...
#define LL_EXTI_ReadReg(__REG__) READ_REG (EXTI->__REG__)
    ...
    /**
     * @brief  Indicate if ExtiLine Interrupt request is enabled for Lines in
     * range 0 to 31
     * @note The reset value for the direct or internal lines (see RM)
     *       is set to 1 in order to enable the interrupt by default.
     *       Bits are set automatically at Power on.
    ...
    /**
     * @brief  Indicate if ExtiLine Event request is enabled for Lines in range
     * 0 to 31
     ...
     * interrupt line occurs during a write operation in the EXTI_RTSR register,
     * the pending bit is not set. Rising and falling edge triggers can be set
     * for the same interrupt line. In this case, both generate a trigger
     *       condition.
     * @rmtoll RTSR        RTx           LL_EXTI_EnableRisingTrig_0_31
     * @param  ExtiLine This parameter can be a combination of the following
     * values:
     *         @arg @ref LL_EXTI_LINE_0
     *         @arg @ref LL_EXTI_LINE_1
     *         @arg @ref LL_EXTI_LINE_2
    ...
    /**
     * @brief  Check if rising edge trigger is enabled for Lines in range 0 to
     * 31
    ...
    /** @defgroup EXTI_LL_EF_Falling_Trigger_Management
     ...
     * @note The configurable wakeup lines are edge-triggered. No glitch must be
     *       generated on these lines. If a falling edge on a configurable
     * interrupt line occurs during a write operation in the EXTI_FTSR register,
     * the pending bit is not set. Rising and falling edge triggers can be set
     * for the same interrupt line. In this case, both generate a trigger
     *       condition.
    ...
    /**
     * @brief  Check if falling edge trigger is enabled for Lines in range 0 to
     * 31
     ...
     * @note If the interrupt is enabled on this line in the EXTI_IMR, writing a
     * 1 to this bit when it is at '0' sets the corresponding pending bit in
     * EXTI_PR resulting in an interrupt request generation. This bit is cleared
     * by clearing the corresponding bit in the EXTI_PR register (by writing a 1
     * into the bit)
    ...
    /**
     * @brief  Check if the ExtLine Flag is set or not for Lines in range 0 to
     * 31
     * @note This bit is set when the selected edge event arrives on the
